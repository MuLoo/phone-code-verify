...
export default function Root(props: PhoneCodeVerifyProps) {
  ...
  const allRefs = Array.from({ length: 6 })
    .map((_, i) => i)
    .map((_) => useRef(null));
  ...
  const handleInput = (e: InputEvent) => {
    ...
    if (
      (nextInput &&
        ...
        (pattern && new RegExp(pattern).test(input.value)) || !pattern)
    ) {
      nextInput.focus();
      if (nextInput.value) {
        nextInput.select();
  ...
  const handlePaste = (e: ClipboardEvent) => {
    ...
    inputRefsArr.forEach((input, i) => {
      if ((pattern && new RegExp(pattern).test(paste[i])) || !pattern) {
  ...
  return (
    <div className={`PCV_container ${wrapperClass}`}>
      <form ref={formRef} style={{ maxWidth }}>
        ...
        <span
          style={{ height: showPatternMessage ? 20 : 0 }}
          className="PCV_patternMessage"
        >
          {patternMessage}
        </span>
        {button && (
          <button
            ...
            disabled={showPatternMessage || !isComplete}
            ref={submitButtonRef}
            type="button"
            className="PCV_submitButton"
          >
